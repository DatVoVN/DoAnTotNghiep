...
const ProfilePage = () => {
  ...
  const handleDeleteCv = async () => {
    ...
    try {
      const response = await fetch(
        "http://localhost:8000/api/candidate/delete-cv",
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-md">
              ...
              <input
                ...
                className="hidden"
                // Vô hiệu hóa khi đang upload HOẶC xóa
                disabled={isUploadingCv || isDeletingCv}
              />

              {/* Hiển thị CV đã có hoặc nút upload */}
              {candidateData?.cvUrl ? (
                // --- Phần hiển thị khi đã có CV ---
                <div
                  className={`p-3 border border-gray-200 rounded-md ${
                    // Làm mờ khi đang upload HOẶC xóa
                    isUploadingCv || isDeletingCv ? "opacity-50" : ""
                  }`}
                >
                  {/* Sử dụng flex và justify-between để căn chỉnh */}
                  <div className="flex items-center justify-between gap-2">
                    {" "}
                    {/* Thêm gap-2 để có khoảng cách nếu wrap */}
                    {/* Thông tin file CV */}
                    <div className="flex items-center gap-2 overflow-hidden mr-2">
                      {" "}
                      {/* mr-2 để tạo khoảng cách với nút */}
                      ...
                      >
                        {/* Hiển thị tên file (có thể cắt bớt nếu quá dài) */}
                        {candidateData.cvUrl
                          .split("/")
                          .pop()
                          ?.substring(0, 30) +
                    ...
                    <div className="flex items-center flex-shrink-0 space-x-3">
                      {" "}
                      {/* Dùng space-x để tạo khoảng cách giữa các nút */}
                      {/* Nút Thay đổi */}
                      <button
                        onClick={handleCvChangeClick}
                        disabled={isUploadingCv || isDeletingCv} // Vô hiệu hóa khi upload hoặc xóa
                      ...
                      {/* --- NÚT XÓA CV ĐƯỢC THÊM VÀO ĐÂY --- */}
                      <button
                        onClick={handleDeleteCv} // Gọi hàm xóa khi click
                        disabled={isUploadingCv || isDeletingCv} // Vô hiệu hóa khi upload hoặc xóa
                        className="text-sm text-red-600 hover:underline disabled:opacity-50 disabled:cursor-not-allowed" // Màu đỏ cho hành động xóa
                      >
                        {isDeletingCv ? "Đang xóa..." : "Xóa"}{" "}
                        {/* Hiển thị text tương ứng trạng thái */}
                      </button>
                      {/* --- KẾT THÚC NÚT XÓA --- */}
                    </div>
                  </div>
              ...
              {/* Hiển thị lỗi upload hoặc xóa CV */}
              {cvError && (
