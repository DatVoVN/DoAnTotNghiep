            ...
            >
              ...
              <div className="flex flex-col items-center md:items-start gap-3 min-w-[40%] md:min-w-[200px]">
                ...
                <div className="relative w-[80px] h-[80px] group">
                                    <img
                                        // Luôn hiển thị ảnh mới nhất từ candidateData
                                        src={candidateData?.avatarUrl ? `http://localhost:8000${candidateData.avatarUrl}?${Date.now()}` : "/img/avatar.jpg"} // Thêm timestamp để tránh cache
                                        alt="avatar"
                                        className="rounded-full w-full h-full object-cover border border-gray-200"
                                        onError={(e) => { e.target.onerror = null; e.target.src='/img/avatar.jpg'}} // Fallback nếu ảnh lỗi
                                    />
                                    {/* Overlay */}
                                    <div
                                        className={`absolute inset-0 bg-black/30 rounded-full flex items-center justify-center transition-opacity ${isUploadingAvatar ? 'opacity-100 cursor-wait' : 'opacity-0 group-hover:opacity-100 cursor-pointer'}`}
                                        onClick={handleAvatarClick}
                                        title={isUploadingAvatar ? "Đang tải lên..." : "Thay đổi ảnh đại diện"}
                                    >
                                        {isUploadingAvatar ? (
                                            <svg className="animate-spin h-6 w-6 text-white" ...> {/* Spinner SVG */} </svg>
                                        ) : (
                                ...
                                {/* Avatar Error */}
                                {avatarError && <p className="text-xs text-red-500 text-center mt-1">{avatarError}</p>}

                                {/* Trạng thái tìm việc, Tên, Địa chỉ (logic isEditing giữ nguyên) */}
                                {/* ... */}
                            </div>
                {/* Trạng thái tìm việc */}
