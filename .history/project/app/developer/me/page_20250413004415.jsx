...
const ProfilePage = () => {
  ...
  const handleAvatarClick = () => {
    // Kích hoạt click cho input file ẩn
  ...
  const handleFileChange = (event) => {
    ...
    if (file) {
      // Kiểm tra loại file (tùy chọn phía client)
      ...
      // Kiểm tra kích thước file (tùy chọn phía client)
      ...
      setAvatarFile(file);
      setAvatarError(null); // Xóa lỗi cũ
      // Tự động upload ngay sau khi chọn file
      handleAvatarUpload(file);
      event.target.value = null; // Reset input file để có thể chọn lại cùng file
    }
  };
