...
const ProfilePage = () => {
  ...
  const handleDeleteCv = async () => {
    ...
    // --- Quan trọng: Kiểm tra xem có token không ---
    if (!token) {
    ...
    try {
      ...
      const response = await fetch("/api/candidate/delete-cv", {
        // Đường dẫn tương đối (cần proxy nếu khác port)
        // Hoặc URL đầy đủ: const response = await fetch('http://localhost:8000/api/candidate/delete-cv', {
        method: "DELETE",
        headers: {
          // GỬI TOKEN XÁC THỰC
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json", // Có thể bỏ qua nếu không gửi body
        },
      });
      // --- Kết thúc gọi API ---

      const responseData = await response.json(); // Luôn cố gắng đọc JSON response

      if (!response.ok) {
        // Ném lỗi với message từ server nếu có
        throw new Error(
          responseData.message || `Lỗi ${response.status}: Không thể xóa CV.`
        );
      }

      // Xóa thành công (status 200 OK)
      console.log("Server response:", responseData.message); // Log message từ server

      // Cập nhật state local để xóa cvUrl khỏi candidateData
      setCandidateData((prevData) => {
        if (!prevData) return null;
        // Tạo object mới không có cvUrl để trigger re-render
        const { cvUrl, ...rest } = prevData;
        return rest;
        // Hoặc: return { ...prevData, cvUrl: undefined }; // Tùy cách bạn muốn quản lý state
      });

      // alert("Đã xóa CV thành công!"); // Có thể thêm thông báo nếu muốn
    } catch (error) {
      console.error("Lỗi khi xóa CV:", error);
      setCvError(error.message || "Đã xảy ra lỗi không mong muốn khi xóa CV.");
    } finally {
      setIsDeletingCv(false); // Luôn kết thúc trạng thái xóa
    }
  };
  const menuItems = [
        ...
        <div className="w-full">
          ...
          <div className="flex flex-col gap-4">
            ...
            >
              ...
              <div className="flex flex-col items-center md:items-start gap-3 min-w-[40%] md:min-w-[200px]">
                ...
                <div className="relative w-[80px] h-[80px] group">
                  {" "}
                  {/* Thêm group để kích hoạt hiệu ứng hover cho phần tử con */}
                  <img
                    // 1. Hiển thị ảnh từ DB hoặc ảnh mặc định:
                    //    - Sử dụng candidateData.avatarUrl nếu có.
                    //    - Nối với base URL của backend (http://localhost:8000) nếu avatarUrl chỉ là đường dẫn tương đối (/uploads/...).
                    //    - Thêm ?{Date.now()} để tránh trình duyệt cache ảnh cũ sau khi upload thành công.
                    //    - Nếu không có avatarUrl, hiển thị ảnh mặc định /img/avatar.jpg.
                    src={
                    ...
                    alt="avatar"
                    // 2. Styling cho ảnh: bo tròn, chiếm hết kích thước, cắt ảnh vừa vặn, thêm viền nhẹ
                    className="rounded-full w-full h-full object-cover border border-gray-200"
                    // 3. Fallback: Nếu ảnh từ server bị lỗi không tải được, hiển thị ảnh mặc định
                    onError={(e) => {
                      e.target.onerror = null;
                      e.target.src = "/img/avatar.jpg";
                    }}
                  />
                  {/* 4. Overlay và Nút bấm (hiển thị khi hover hoặc đang upload) */}
                  <div
                    // Styling Overlay:
                    // - absolute inset-0: Phủ kín thẻ div cha (relative).
                    // - bg-black/30: Nền đen mờ.
                    // - rounded-full: Bo tròn giống ảnh.
                    // - flex items-center justify-center: Căn giữa icon bên trong.
                    // - transition-opacity: Tạo hiệu ứng mờ dần.
                    // - opacity-0 group-hover:opacity-100: Mặc định ẩn, hiện khi hover vào thẻ div cha (nhờ class 'group').
                    // - !opacity-100 cursor-wait: Luôn hiện và đổi cursor khi đang upload (isUploadingAvatar).
                    // - cursor-pointer: Đổi cursor khi không upload.
                    className={`absolute inset-0 bg-black/30 rounded-full flex items-center justify-center transition-opacity ${
                      isUploadingAvatar
                        ? "!opacity-100 cursor-wait"
                        : "opacity-0 group-hover:opacity-100 cursor-pointer"
                    }`}
                    // 5. Kích hoạt chọn file khi click vào overlay (nếu không đang upload)
                  ...
                  >
                    {/* 6. Hiển thị Spinner hoặc Icon Camera */}
                    {isUploadingAvatar ? (
                      // Spinner SVG khi đang tải
                      <svg
                      ...
                      // Icon camera khi không tải
                      <i className="bi bi-camera text-white text-2xl"></i>
                    )}
                  </div>
                  {/* 7. Input type="file" ẩn */}
                  <input
                    type="file"
                    accept="image/png, image/jpeg, image/gif" // Chỉ chấp nhận các định dạng ảnh phổ biến
                    ref={fileInputRef} // Gắn ref để có thể trigger click từ code
                    onChange={handleFileChange} // Gọi hàm xử lý khi file được chọn
                    className="hidden" // Luôn ẩn input này khỏi giao diện
                    disabled={isUploadingAvatar} // Vô hiệu hóa khi đang upload
                  />
                </div>
                {/* 8. Hiển thị lỗi upload avatar (nếu có) */}
                {avatarError && (
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-md">
              ...
              <input
                type="file"
                accept=".pdf" // Chỉ chấp nhận PDF cho CV
                ref={cvInputRef}
                onChange={handleCvFileChange}
                className="hidden"
                disabled={isUploadingCv}
              />
              {/* Hiển thị CV đã có hoặc nút upload */}
              {candidateData?.cvUrl ? (
                <div
                  className={`p-3 border border-gray-200 rounded-md ${
                    isUploadingCv ? "opacity-50" : ""
                  }`}
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2 overflow-hidden">
                      ...
                      <a
                        href={`http://localhost:8000${
                          candidateData.cvUrl
                        }?${Date.now()}`} // Thêm timestamp
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-sm text-blue-600 hover:underline truncate"
                        title={candidateData.cvUrl.split("/").pop()} // Tooltip cho tên file đầy đủ
                      >
                    ...
                    <button
                      ...
                      disabled={isUploadingCv}
                      className="text-sm text-purple-600 hover:underline flex-shrink-0 ml-2 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
