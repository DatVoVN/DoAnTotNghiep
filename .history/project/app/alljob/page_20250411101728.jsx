...
const page = () => {
  return <div className="max-w-screen-xl mx-auto px-4"
  <div className="bg-primary px-12 py-8 rounded-lg">
  <div className="w-full flex items-center text-sm relative flex-wrap gap-2">
      ...
      <div className="hidden w-full mt-0 bg-white absolute top-[45px] lg:top-14 left-0 rounded-lg border border-gray-200 px-4 py-2 z-[3]">
        <div className="grid grid-cols-2">
          <div className="col-span-2">
            <div className="text-purple-600 text-base font-bold mb-2">Từ khóa phổ biến</div>
            <ul>
              {["Kế toán", "marketing", "Lái xe", "Chăm sóc khách hàng", "thực tập sinh"].map((item, i) => (
                <li
                  key={i}
                  className="px-1 py-2 rounded hover:bg-purple-100 cursor-pointer"
                >
                  <div className="text-gray-700 font-bold">
                    <i className="svicon-star mr-2 text-yellow-400"></i>{item}
                  </div>
                </li>
              ))}
            </ul>
    ...
    {/* Bộ lọc nghề nghiệp (giả lập select-react) */}
    ...
    {/* Lọc nâng cao */}
    <div className="bg-lime-600 hover:bg-lime-700 transition rounded-sm cursor-pointer h-12 w-40">
      <div className="w-full h-full flex items-center justify-center text-white text-sm font-semibold p-2">
        Lọc nâng cao
      </div>
    </div>
  </div>
