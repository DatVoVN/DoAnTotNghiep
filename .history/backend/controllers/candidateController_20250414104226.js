...
// Ví dụ kiểm tra hàm async và candidate lookup
const deleteCV = async (req, res) => {
  try {
    console.log("User from request:", req.user); // Check user object
    if (!req.user || !req.user.id) {
      return res.status(401).json({ message: "Xác thực không hợp lệ." });
    }
    const userId = req.user.id;

    // ---- THAY ĐỔI CÁCH TÌM KIẾM NẾU CẦN ----
    // const candidate = await Candidate.findById(userId); // Nếu userId là Candidate._id
    const candidate = await Candidate.findOne({ userId: userId }); // Nếu Candidate có trường userId tham chiếu đến User
    // -----------------------------------------

    console.log("Candidate found:", candidate); // Check if candidate is found
    ...
    console.log("Current CV Path from DB:", currentCvPath); // Check the path

    candidate.cvUrl = undefined;
    // candidate.updatedAt = Date.now(); // Có thể bỏ nếu schema có timestamps: true
    await candidate.save();
    ...
    try {
      ...
      // Nếu deleteFileIfExists throw error thay vì trả về false
      console.error("Error thrown by deleteFileIfExists:", fileError);
      deleted = false; // Coi như xóa file thất bại
    }
    // -----------------------------------------
    ...
    if (deleted) {
      ...
      // Dù file không xóa được, DB đã cập nhật, vẫn trả về 200 với cảnh báo
      console.warn(
