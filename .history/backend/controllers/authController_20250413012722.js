...
const authController = {
  ...
  resendOtp: async (req, res) => {
    ...
    try {
      ...
      if (!emailSent) {
        ...
        return res.status(500).json({
          message: "Lỗi hệ thống khi gửi lại email xác thực. Vui lòng thử lại.",
        });
      }
  ...
  loginCandidate: async (req, res) => {
    try {
      ...
      if (!candidate) {
        // Trả về thông báo chung để không tiết lộ email nào tồn tại
        return res
          .status(400)
      ...
      if (!candidate.isVerified) {
        ...
        // Nếu tài khoản chưa được xác thực, trả về lỗi 403 (Forbidden)
        // Kèm theo thông báo rõ ràng cho người dùng và gợi ý cho frontend
        return res.status(403).json({
          message:
            "Tài khoản của bạn chưa được xác thực. Vui lòng kiểm tra email để lấy mã OTP hoặc yêu cầu gửi lại mã.",
          // Cờ này giúp frontend biết cần hiển thị form nhập OTP thay vì báo lỗi chung chung
          verificationRequired: true,
          email: normalizedEmail, // Trả lại email để frontend có thể dùng cho việc gửi lại OTP
        });
      }
      // *** KẾT THÚC KIỂM TRA ***

      // Nếu đã xác thực, kiểm tra mật khẩu
      ...
      // Nếu mật khẩu khớp và đã xác thực, tạo token
      const token = jwt.sign(
        { id: candidate._id, email: candidate.email, role: "candidate" },
        process.env.JWT_SECRET, // Đảm bảo dùng đúng tên biến môi trường JWT_SECRET
      ...
      res.status(200).json({
        ...
        candidate: {
          ...
          avatarUrl: candidate.avatarUrl, // Có thể trả về avatar nếu cần
          // Thêm các trường khác nếu frontend cần ngay sau khi login
        },
      });
    } catch (error) {
