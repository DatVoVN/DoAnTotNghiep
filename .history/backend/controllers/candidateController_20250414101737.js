...
const deleteCV = async (req, res) => {
  try {
    ...
    // Kiểm tra xem ứng viên có CV để xóa không
    if (!candidate.cvUrl) {
      return res.status(400).json({ message: "Không có CV nào để xóa." });
    }

    // Lấy đường dẫn tương đối của CV hiện tại
    const currentCvPath = candidate.cvUrl;

    // Cập nhật DB trước khi xóa file vật lý (an toàn hơn)
    candidate.cvUrl = undefined; // Hoặc null, tùy vào schema/logic của bạn
    candidate.updatedAt = Date.now(); // Cập nhật thời gian
    await candidate.save();

    // Xóa file CV vật lý bằng helper function
    const deleted = deleteFileIfExists(currentCvPath);

    if (deleted) {
      res.status(200).json({ message: "Xóa CV thành công!" });
    } else {
      // Mặc dù DB đã được cập nhật, thông báo rằng file không tìm thấy hoặc có lỗi khi xóa
      res.status(200).json({
        message:
    ...
    // Xử lý lỗi Validation nếu có (ít khả năng xảy ra khi chỉ set undefined)
    if (err.name === "ValidationError") {
      const errors = Object.values(err.errors).map((el) => ({
        field: el.path,
        message: el.message,
